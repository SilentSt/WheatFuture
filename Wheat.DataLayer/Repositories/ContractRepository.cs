...
namespace Wheat.DataLayer.Repositories
{
    public class ContractRepository : IContractRepository
    {
        ...
        public async Task<Contracts> GetMyBuyContracts(string userId)
        {
            ...
            {
                return x.GroupBy(f => f.OldPrice).Select(g => new PriceRow
                {
                    Code = x.Key,
        ...
        public async Task ReSellContractsAsync(string userId, string code, int count, double price, double newPrice)
        {
            ...
            if (seller == null) throw new Exception("404 Seller not found");
            if (user.SelfBuyContracts.Count(x => x.Price == price && x.SellerCode == seller.Code && x.OnTrade == false) >= count)
            {
        ...
        public async Task<BalanceResults> GetTotalBalanceAsync(string userId)
        {
            ...
            var operations = _context.OperationResults.Where(x => x.UserId == userId);
        ...
        public async Task<List<ContractPrice>> GetPublicSellContracts()
        {
            var result = _context.SellContracts.Where(x => x.OnTrade && x.SellerCode != null).GroupBy(f => f.OldPrice).Select(g => new ContractPrice
            {
                Count = _context.SellContracts.Count(y => y.OldPrice == g.Key),
                Contract = g.First()
            }).ToList();
            return result;
        }

        public async Task<List<ContractPrice>> GetPublicBuyContracts()
        {
            var result = _context.SellContracts.Where(x => x.OnTrade && x.SellerCode == null).GroupBy(f => f.OldPrice).Select(g => new ContractPrice
            {
                Count = _context.SellContracts.Count(y => y.OldPrice == g.Key),
